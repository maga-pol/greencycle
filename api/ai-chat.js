...
module.exports = async function handler(req, res) {
  ...
  try {
        ...
        systemInstruction: {
          parts: [{
            text: [
              ...
              "Никогда не начинай ответ с приветствия, самопрезентации или фраз вроде: 'Привет', 'Здравствуйте', 'Я Migo', 'На связи Migo', 'рад помочь', 'с удовольствием расскажу'. Сразу отвечай на вопрос.",
              ...
              "Пиши простым языком, обычно 3-5 коротких абзацев или короткий список. Первый абзац должен содержать прямой ответ на вопрос пользователя.",
              "В конце задай максимум 1-2 уточняющих вопроса только если они реально нужны.",
              PLANT_CONTEXT
            ].join("\n\n")
          }]
        },
    ...
    return res.status(200).json({
      reply: cleanReply(reply) || "Migo не смог сформировать ответ. Попробуйте переформулировать вопрос."
    });
  } catch (error) {
...
function cleanReply(text) {
  return String(text || "")
    .replace(/^\s*(привет|здравствуйте|добрый день|добрый вечер|конечно)[!.,:\-\s]*/i, "")
    .replace(/^\s*(я\s+)?(migo|миго)[^.!?\n]*(помощник|связи|рад|рада|расскажу)[.!?\n]\s*/i, "")
    .replace(/^\s*(твой|ваш)\s+[^.!?\n]*(помощник|мире растений|здоровой почвы)[.!?\n]\s*/i, "")
    .trim();
}
